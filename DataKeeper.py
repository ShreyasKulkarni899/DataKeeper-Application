...
TABLENAMEUT =StringVar() ; COLUMNNAMEUT = StringVar()
TABLENAMEDT =StringVar() ;  TUPLENAME = StringVar() ; TUPCOLUMNNAME =StringVar()
TABLENAMEIU = StringVar() ; COLUMNNAMEIU = StringVar() ; RANDOM = StringVar() ; RANDOM1 = StringVar(); COLUMNNAMEIUVALUE = StringVar()
TABLENAMEST = StringVar() ; RANDOM2 = StringVar() ; COLUMNNAMEST = StringVar()
...
def Database():
    global conn, cursor 
    conn = mysql.connector.connect(host="localhost",username="root",password="root",database="mydb")
...
def BasicInformation():
    ...
    top1.grid(row=0)
    top2 = Label(top,text="Backgrong Language - MySQL ",font=('times new roman',20),bg="black",fg="white")
    top2.grid(row=1)
...
def Login():
    ...
    if username == "" or password == "" or captcha == "":
        ...
        cursor.execute("SELECT * FROM member WHERE username = %s and password = %s",(str(username),str(password)))
...
def ForgetPassword():
    ...
    if username_info == "":
        ...
        cursor.execute("SELECT * FROM member WHERE username = %s ",(username_info,))
...
def password_updater(username_info, password_info):
    ...
    cursor.execute("UPDATE member SET password = %s WHERE username = %s ",
                   (str(password_demo),str(username_info)))
...
def UpdateTableForm():
    global UpdateTabFrame, lbl_result_UT , TbName ,Clname
    ...
    '''btn_TB = Button(UpdateTabFrame0, text="SELECT TABLE",font=("times new roman",18),bg="green",fg="white",bd=5,activebackground="seagreen1",cursor="hand2", width=35,
                                command= lambda : Tb_Show(UpdateTabFrame0) )
    btn_TB.grid(row=1, column =0, pady=20)

    btn_CL = Button(UpdateTabFrame0, text="SELECT COLUMN",font=("times new roman",18),bg="green",fg="white",bd=5,activebackground="seagreen1",cursor="hand2", width=35,
                                command= lambda : Cl_Show(UpdateTabFrame0) )
    btn_CL.grid(row=1,column =1, pady=20)'''
...
def Attr_Show(UpdateTabFrame0):
    ...
    btn_SUBMIT = Button(UpdateTabFrame2, text="SUBMIT",font=("times new roman",18),bg="green",fg="white",bd=5,activebackground="seagreen1",cursor="hand2", width=20,
                                command= ModifyTableUT )

    btn_SUBMIT.grid(row=1,column =3, pady=20)
    ...
    btn_CANCEL.grid(row=1,column =1, pady=20)
...
def GetColumnNamesUT():
    ...
    cursor.execute(getColumnNames_msg)
...
def ModifyTableUT():
    Database()
    #msg = AtrUT()
    final_msg = f"ALTER TABLE  {TABLENAMEUT.get()} MODIFY {COLUMNNAMEUT.get()} {AUTDTYPE.get()}({AUTDLENGTH.get()})"
    print(final_msg)
...
def DeleteTableForm():
    global DeleteTabFrame, lbl_result_DT , TbName
...
def GetTableDT():
    Database()
    tab_name = TABLENAMEDT.get()
    getTableNames_msg = f"SELECT * FROM {tab_name}"
    cursor = conn.execute(getTableNames_msg)
    names = [description[0] for description in cursor.description]
    return tuple(names)  

#delete table method
def Tb_ShowD(DeleteTabFrame0):
    ...
    TbName.grid( row = 2, column = 1) 
    TbName.current()
    btn_TBD = Button(DeleteTabFrame0, text="DROP",font=("times new roman",18),bd=2,activebackground="seagreen1",cursor="hand2", width=10,command= DropTable )
    btn_TBD.grid(row=2 ,column =2, pady=20)

# delete table method
def DropTable():
    Database()
    msg = f"DROP TABLE mydb.{TABLENAMEDT.get()}"
    print(msg)
    cursor.execute(msg)
    conn.commit()
    conn.close()
...
def InsertTupleForm():
    global InsertTupFrame, lbl_result_IU , TbNameIU ,ClnameIU,Data
...
# update table method
def Tb_ShowIU(InsertTupFrame0):
    Label(InsertTupFrame0, text="SELECT TABLE NAME:").grid(row = 2,column=0)
    global btn_TB 
    btn_TB = Button(InsertTupFrame0, text="NEXT",font=("times new roman",18),bd=2,activebackground="seagreen1",cursor="hand2", width=10,command= lambda:GetColumnNamesIU(InsertTupFrame0) )
    btn_TB.grid(row=2 ,column =2, pady=20)
    TbNameIU = ttk.Combobox(InsertTupFrame0, textvariable = TABLENAMEIU ) 
    TbNameIU['values'] = DropDownlist()
    TbNameIU.grid( row = 2, column = 1) 
    TbNameIU.current() 

def GetColumnNamesIU(InsertTupFrame0):
    Database()
    tab_name = TABLENAMEIU.get()
    getColumnNames_msg = f" SELECT * FROM {tab_name}"
    print(getColumnNames_msg)
    cursor.execute(getColumnNames_msg)
    names = [description[0] for description in cursor.description]
    ...
    name1=names[0]
    Label(InsertTupFrame0, text=name1).grid(row = 4  ,column=0)
    data= Entry(InsertTupFrame0 ,text="", font=('times new roman', 20),bg="lavender", textvariable=RANDOM, width=20).grid(row = i1+4 , column=1)
    data1= Entry(InsertTupFrame0 ,text="", font=('times new roman', 20),bg="lavender", textvariable=RANDOM, width=20).grid(row = i1+4 , column=1)
    btn = Button(InsertTupFrame0, text="SHOW",font=("times new roman",18),bg="green",fg="white",bd=2,activebackground="seagreen1",cursor="hand2", width=10,command=setValue(RANDOM,name1,names))
    btn.grid(row=4 ,column =2, pady=20)
    name2=names[1]
    Label(InsertTupFrame0, text=name2).grid(row = 5  ,column=0)
    data= Entry(InsertTupFrame0 ,text="", font=('times new roman', 20),bg="lavender", textvariable=RANDOM1, width=20).grid(row = i1+4 , column=1)
    data1= Entry(InsertTupFrame0 ,text="", font=('times new roman', 20),bg="lavender", textvariable=RANDOM1, width=20).grid(row = i1+4 , column=1)
    btn = Button(InsertTupFrame0, text="SHOW",font=("times new roman",18),bg="green",fg="white",bd=2,activebackground="seagreen1",cursor="hand2", width=10,command=setValue(RANDOM1,name2,names))
    btn.grid(row=5 ,column =2, pady=20)
      

def setValue(RANDOM,name,names):
    Database()
    name1= str(name)
    random =str(RANDOM.get())
    table_name1=str(TABLENAMEIU.get())    
    if name1==names[0]:
        msg=f'''INSERT INTO {table_name1}({name1}) VALUES('{random}')'''
        cursor.execute(msg)
        COLUMNNAMEIU.set(name1)
        COLUMNNAMEIUVALUE.set(random)
        RANDOM.set("")
        conn.commit()
        conn.close()
    else:
        columnIU = str(COLUMNNAMEIU.get())
        value = str(COLUMNNAMEIUVALUE.get())
        value1 = str(RANDOM1.get())
        #msg=f'''UPDATE {table_name1} SET {name1} = {value1} WHERE {columnIU} = ('{value}')'''
        cursor.execute('`UPDATE` {table_name1} `SET` {name1} = {value1} `WHERE` {columnIU} = (%s)' %(value)) 
        RANDOM1.set("")
        conn.commit()
        conn.close()
...
def DeleteTupleForm():
    global DeleteTupFrame, lbl_result_IU , TbNameIU ,ClnameIU,Data
    DeleteTupFrame = Frame(root)
    DeleteTupFrame.pack(fill = "both" , expand = True , anchor = 'c', pady =40 , padx = 40)
    DeleteTupFrame0 = Frame(DeleteTupFrame)
    DeleteTupFrame0.pack(side=TOP, pady=20 , anchor = 'c')
    lbl_header = Label(DeleteTupFrame0 , text = "          Delete Tuple         ", font=('helvetica 16 bold italic', 26), bd=22 ,bg= "light blue")
    lbl_header.grid(row = 0,columnspan =2)
    Label(DeleteTupFrame0, text="     ").grid(row = 1)
    Tb_ShowDT(DeleteTupFrame0)
    lbl_Back = Label(DeleteTupFrame0, text="BACK", fg="brown4",cursor="hand2", font=('algerian', 12,"bold underline"))
    lbl_Back.grid(row=0, sticky=W)
    lbl_Back.bind('<Button-1>', Toggle_FromDeleteTup_ToNaviagation)
    
# update table method
def Tb_ShowDT(DeleteTupFrame0):
    Label(DeleteTupFrame0, text="SELECT TABLE NAME:").grid(row = 2,column=0)
    global btn_TB 
    btn_TB = Button(DeleteTupFrame0, text="NEXT",font=("times new roman",18),bd=2,activebackground="seagreen1",cursor="hand2", width=10,command= lambda:GetColumnNamesDT(DeleteTupFrame0) )
    btn_TB.grid(row=2 ,column =2, pady=20)
    TbNameIU = ttk.Combobox(DeleteTupFrame0, textvariable = TABLENAMEIU ) 
    TbNameIU['values'] = DropDownlist()
    TbNameIU.grid( row = 2, column = 1) 
    TbNameIU.current() 

def GetColumnNamesDT(DeleteTupFrame0):
    Database()
    tab_name = TABLENAMEIU.get()
    getColumnNames_msg = f" SELECT * FROM {tab_name}"
    print(getColumnNames_msg)
    cursor.execute(getColumnNames_msg)
    names = [description[0] for description in cursor.description]
    firstAttribute = str(names[0])
    Show_DT(DeleteTupFrame0,firstAttribute,tab_name)
    

def Show_DT(DeleteTupFrame0,firstAttribute,tab_name):
    Database()
    gettup = f'SELECT * FROM {tab_name}'
    cursor.execute(gettup)
    names1 = []
    records = cursor.fetchall()
    for record in records:
        names1.append(record[0])
    Label(DeleteTupFrame0, text="SELECT Attribute NAME:").grid(row = 3,column=0)
    btn_DT = Button(DeleteTupFrame0, text=" Delete the Tuple",font=("times new roman",18),bd=2,activebackground="seagreen1",cursor="hand2", width=15,command= lambda:Delete_Backend(tab_name,firstAttribute) )
    btn_DT.grid(row=5 ,column =1, pady=20)
    TbNameDT = ttk.Combobox(DeleteTupFrame0, textvariable = TUPLENAME ) 
    TbNameDT['values'] = names1
    TbNameDT.grid( row = 3, column = 1) 
    TbNameDT.current()   

def Delete_Backend(tab_name,firstAttribute):
    Database()
    value = str(TUPLENAME.get())
    query = f"""DELETE FROM {tab_name} WHERE {firstAttribute} IN ("{value}") """
    #query = f"""DELETE FROM {tab_name} WHERE {firstAttribute} IN ("{value}") """
    print(query,value)
...
#=================================================================update tuple Form ==============================================================================================
def UpdateTupleForm():
    global UpdateTupFrame, lbl_result_IU , TbNameIU ,ClnameIU,Data
    UpdateTupFrame = Frame(root)
    UpdateTupFrame.pack(fill = "both" , expand = True , anchor = 'c', pady =40 , padx = 40)
    UpdateTupFrame0 = Frame(UpdateTupFrame)
    UpdateTupFrame0.pack(side=TOP, pady=20 , anchor = 'c')
    lbl_header = Label(UpdateTupFrame0 , text = "          Update Tuple         ", font=('helvetica 16 bold italic', 26), bd=22 ,bg= "light blue")
    lbl_header.grid(row = 0,columnspan =2)
    Label(UpdateTupFrame0, text="     ").grid(row = 1)
    Tb_ShowUP(UpdateTupFrame0)
    lbl_Back = Label(UpdateTupFrame0, text="BACK", fg="brown4",cursor="hand2", font=('algerian', 12,"bold underline"))
    lbl_Back.grid(row=0, sticky=W)
    lbl_Back.bind('<Button-1>', Toggle_FromUpdateTup_ToNaviagation)
    
# update table method

#-------------------------------------------------------------- Show Table Name -----------------------------------------------#
def Tb_ShowUP(UpdateTupFrame0):
    Label(UpdateTupFrame0, text="SELECT TABLE NAME:").grid(row = 2,column=0)
    global btn_TB 
    # btn_TB = Button(UpdateTupFrame0, text="NEXT",font=("times new roman",18),bd=2,activebackground="seagreen1",cursor="hand2", width=10,command= lambda:GetColumnNamesUP(UpdateTupFrame0) )
    # btn_TB.grid(row=2 ,column =2, pady=20)
    btn_TB = Button(UpdateTupFrame0, text="NEXT",font=("times new roman",18),bd=2,activebackground="seagreen1",cursor="hand2", width=10,command= lambda:Show_UP(UpdateTupFrame0) )
    btn_TB.grid(row=2 ,column =2, pady=20)
    TbNameIU = ttk.Combobox(UpdateTupFrame0, textvariable = TABLENAMEIU ) 
    TbNameIU['values'] = DropDownlist()
    TbNameIU.grid( row = 2, column = 1) 
    TbNameIU.current()
    TbNameIU.config(font=35) 
#-------------------------------------------------------------- Show Column Name -----------------------------------------------#

def Show_UP(UpdateTupFrame0):
    Database()
    tab_name = TABLENAMEIU.get()
    getColumnNames_msg = f" SELECT * FROM {tab_name}"
    print(getColumnNames_msg)
    cursor.execute(getColumnNames_msg)
    names = [description[0] for description in cursor.description]
    Label(UpdateTupFrame0, text="SELECT COLUMN:").grid(row = 3,column=0)
    TbNameDT = ttk.Combobox(UpdateTupFrame0, textvariable = TUPCOLUMNNAME ) 
    TbNameDT['values'] = names
    TbNameDT.grid( row = 3, column = 1)
    TbNameDT.set("*") 
    TbNameDT.current()
    TbNameDT.config(font= 35)   
    btn_TB = Button(UpdateTupFrame0, text="NEXT",font=("times new roman",18),bd=2,activebackground="seagreen1",cursor="hand2", width=10,command= lambda:GetColumnNamesUP(UpdateTupFrame0))
    btn_TB.grid(row=3 ,column =2, pady=20)           
  
#-------------------------------------------------------------- Show Tuple Name -----------------------------------------------#

def GetColumnNamesUP(UpdateTupFrame0):
    Database()
    tab_name = TABLENAMEIU.get()
    col_name = TUPCOLUMNNAME.get()
    gettup = f'SELECT {col_name} FROM {tab_name}'
    cursor.execute(gettup)
    names1 = []
    records = cursor.fetchall()
    for record in records:
        names1.append(record[0])
    Label(UpdateTupFrame0, text="SELECT TUPLE:").grid(row = 5,column=0)
    TbNameDT = ttk.Combobox(UpdateTupFrame0, textvariable = TUPLENAME ) 
    TbNameDT['values'] = names1
    TbNameDT.grid( row = 5, column = 1) 
    TbNameDT.current()   
    btn_TB = Button(UpdateTupFrame0, text="NEXT",font=("times new roman",18),bd=2,activebackground="seagreen1",cursor="hand2", width=10,command= lambda: newValue(UpdateTupFrame0,tab_name,col_name) )
    btn_TB.grid(row=5 ,column =2, pady=20)    


def newValue(UpdateTupFrame0,tab_name,col_name):
    tuple_name = TUPLENAME.get()
    Label(UpdateTupFrame0, text="Enter the new value").grid(row = 6,column=0)
    newValue = Entry(UpdateTupFrame0 ,text="", font=('times new roman', 20),bg="lavender", textvariable= A10NAME , width=20).grid(row = 6 , column=1) 
    btn_DT = Button(UpdateTupFrame0, text=" Update the Tuple",font=("times new roman",18),bd=2,activebackground="seagreen1",cursor="hand2", width=15,command= lambda:Update_Backend(tab_name,col_name,tuple_name) )
    btn_DT.grid(row=8 ,column =1, pady=20)

def Update_Backend(tab_name,col_name,tuple_name):
    Database()
    query = f"""UPDATE {tab_name} SET {col_name} = "{A10NAME.get()}" WHERE {col_name} = "{tuple_name}" """
    print(query)
    cursor.execute(query)
    conn.commit()
    print("in the backend")
...
#================================================================Show table Form ==============================================================================================
def ShowTableForm():
    global ShowTableFrame, lbl_result_ST , TbNameST ,ClnameST,Data
    ShowTableFrame = Frame(root)
    ShowTableFrame.pack(fill = "both" , expand = True , anchor = 'c', pady =40 , padx = 40)
    ShowTableFrame0 = Frame(ShowTableFrame)
    ShowTableFrame0.pack(side=TOP, pady=20 , anchor = 'c')
    lbl_header = Label(ShowTableFrame0 , text = "          Show Table         ", font=('helvetica 16 bold italic', 26), bd=22 ,bg= "light blue")
    lbl_header.grid(row = 0,columnspan =2)
    Label(ShowTableFrame0, text="     ").grid(row = 1)
    Label(ShowTableFrame0, text="SELECT TABLE NAME:").grid(row = 2,column=0)
    btn_TB = Button(ShowTableFrame0, text="NEXT",font=("times new roman",18),bd=2,activebackground="seagreen1",cursor="hand2", width=10,command= lambda:Show_ST(ShowTableFrame0) )
    btn_TB.grid(row=2 ,column =2, pady=20)
    TbNameIU = ttk.Combobox(ShowTableFrame0, textvariable = TABLENAMEST ) 
    TbNameIU['values'] = DropDownlist()
    TbNameIU.grid( row = 2, column = 1) 
    TbNameIU.current()
    TbNameIU.config(font=35) 
    lbl_Back = Label(ShowTableFrame0, text="BACK", fg="brown4",cursor="hand2", font=('algerian', 12,"bold underline"))
    lbl_Back.grid(row=0, sticky=W)
    lbl_Back.bind('<Button-1>', Toggle_FromShowTable_ToNaviagation)
    
#-------------------------------------------------------------- Show Column Name -----------------------------------------------#

def Show_ST(ShowTableFrame0):
    Database()
    tab_name = TABLENAMEST.get()
    getColumnNames_msg = f" SELECT * FROM {tab_name}"
    print(getColumnNames_msg)
    cursor.execute(getColumnNames_msg)
    names = [description[0] for description in cursor.description]
    Label(ShowTableFrame0, text="SELECT COLUMN:").grid(row = 3,column=0)
    TbNameDT = ttk.Combobox(ShowTableFrame0, textvariable = COLUMNNAMEST ) 
    TbNameDT['values'] = names
    TbNameDT.grid( row = 3, column = 1)
    TbNameDT.set("*") 
    TbNameDT.current()
    TbNameDT.config(font= 35)   
    btn_TB = Button(ShowTableFrame0, text="NEXT",font=("times new roman",18),bd=2,activebackground="seagreen1",cursor="hand2", width=10,command= lambda:ShowTab_Backend(ShowTableFrame0))
    btn_TB.grid(row=3 ,column =2, pady=20)           

def ShowTab_Backend(ShowTableFrame0):
    Database()
    tab_name = TABLENAMEST.get()
    col_name = COLUMNNAMEST.get()
    query = f"""SELECT {col_name} FROM {tab_name} """
    cursor.execute(query)
    i=5 
    for row in cursor: 
        for j in range(len(row)):
            e = Entry(ShowTableFrame0, width=10, fg='blue') 
...
def Toggle_FromNavigation_ToInsertTap(event=None):
    NavigationFrame.destroy()
    InsertTupleForm()
...
def Toggle_FromNavigation_ToDeleteTap(event=None):
    NavigationFrame.destroy()
    DeleteTupleForm()

def Toggle_FromNavigation_ToUpdateTap(event=None):
    NavigationFrame.destroy()
    UpdateTupleForm()    

def Toggle_FromInsertTup_ToNaviagation(event=None):
    InsertTupFrame.destroy()
    NavigationForm()

def Toggle_FromDeleteTup_ToNaviagation(event=None):
    DeleteTupFrame.destroy()
    NavigationForm()

def Toggle_FromUpdateTup_ToNaviagation(event=None):
    UpdateTupFrame.destroy()
    NavigationForm()

def Toggle_FromShowTable_ToNaviagation(event=None):
    ShowTableFrame.destroy()
    NavigationForm()
...
def menubar(status1):
    ...
    if status1 == 1:
        ...
        filemenu1.add_command(label="4.View Table"   , command = Toggle_FromNavigation_ToViewTab)
        filemenu1.add_command(label=" Structure")
        filemenu2.add_command(label="1.Insert Tuple" , command = Toggle_FromNavigation_ToInsertTap)
        filemenu2.add_command(label="2.Update Tuple" , command = Toggle_FromNavigation_ToUpdateTap)
        filemenu2.add_command(label="3.Delete Tuple" , command = Toggle_FromNavigation_ToDeleteTap)
